...
class MemorialStoriesAPI {
  ...
  /**
   * Submit a new story with file upload support
   */
  async submitStory(formData, files = []) {
    try {
      let photoUrls = [];
      ...
      if (files && files.length > 0) {
        ...
        // Add shop domain (available in Liquid context)
        const shop = window.MEMORIAL_SETTINGS?.shop || window.Shopify?.shop;
        if (!shop) {
          console.error('Shop domain not available');
          return { success: false, error: 'Shop domain not available. Please refresh the page.' };
        }
        ...
        if (!uploadData.success) {
          return {
            success: false,
            error: uploadData.error || 'Image upload failed',
      ...
      formData.append("shop", window.MEMORIAL_SETTINGS?.shop || window.Shopify?.shop || "");
...
class StoryWallManager {
  ...
  setupFormSubmission() {
    ...
    form.addEventListener('submit', async (e) => {
      ...
      const result = await this.api.submitStory(formData, selectedFiles);
